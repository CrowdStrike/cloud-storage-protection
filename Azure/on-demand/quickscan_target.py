...
# flake8: noqa
"""Scan an Azure Storage container with the CrowdStrike QuickScan Pro API.

Scans an Azure Storage container using the CrowdStrike Falcon QuickScan Pro APIs.
...
- Recommended deployment in Azure (container, VM, or Function App)
- Files > 256MB are automatically skipped

REQUIREMENTS:
- Target must be a full Azure Storage URL
- Requires Azure SDK and CrowdStrike FalconPy >= v0.8.7
    python3 -m pip install azure-identity azure-storage-blob crowdstrike-falconpy
...
python3 quickscan_target.py -t https://STORAGE_ACCOUNT.blob.core.windows.net/CONTAINER -k API_KEY -s API_SECRET [-b BATCH_SIZE] [-w WORKERS] [-d CHECK_DELAY] [-l LOG_LEVEL]

Example scenarios:
1. Process 1500 files in batches of 500 using 10 workers:
   python3 quickscan_target.py -t https://myaccount.blob.core.windows.net/mycontainer -k API_KEY -s API_SECRET -b 500 -w 10

2. Default processing (1000 file batches, 10 workers):
   python3 quickscan_target.py -t https://myaccount.blob.core.windows.net/mycontainer -k API_KEY -s API_SECRET
"""
import os
import io
import time
